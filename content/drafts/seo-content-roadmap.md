# SEO Content Roadmap — Dr Jeremy Sun Plastic Surgery

Status: internal draft. Do not publish until reviewed and approved.

## Publishing principles

- Patient-facing, educational and factual.
- Avoid guarantees, superlatives, sensational claims, testimonials and before/after promises.
- Use Dr Sun’s objective credentials: Senior Consultant status, academic work, publications, dedicated lymphedema training, invited talks, reconstructive experience.
- Each procedure page should include: overview, who may be suitable, consultation assessment, treatment options, recovery, risks/limitations, FAQs and enquiry CTA.
- Articles should answer specific patient questions and link naturally to the relevant procedure page.

## Priority procedure pages

### Current status

- Published: `Lymphedema Surgery Singapore`
- Published: `Lymphovenous Bypass / LVA Surgery Singapore`
- Published: `Breast Reconstruction Singapore`
- Published: `Asian Eyelid Surgery Singapore`
- Drafted for clinical review: `Rhinoplasty Singapore`
- Drafted for clinical review: `Body Contouring & Liposuction Singapore`
- Drafted for clinical review: `Breast Aesthetic Surgery Singapore`

### Tier 1 — strategic / highest differentiation

1. Lymphedema Surgery Singapore
   - Status: published
   - Target: `lymphedema surgery Singapore`, `lymphatic surgery Singapore`, `lymphovenous bypass Singapore`, `lymphoedema surgeon Singapore`
   - Differentiator: MOH-accredited dedicated Tokyo lymphedema surgery training; invited local/international speaker; academic publications.

2. Lymphovenous Bypass / LVA Surgery Singapore
   - Status: published
   - Target: `lymphovenous bypass Singapore`, `LVA surgery Singapore`, `lymphatic bypass surgery Singapore`
   - Differentiator: technical lymphatic microsurgery focus and academic work.

3. Breast Reconstruction Singapore
   - Status: published
   - Target: `breast reconstruction Singapore`, `DIEP flap Singapore`, `reconstruction after mastectomy Singapore`
   - Differentiator: reconstruction component for Singapore’s first robotic mastectomy; microsurgical reconstruction experience.

### Tier 2 — aesthetic search demand

4. Asian Eyelid Surgery Singapore
   - Status: published
   - Target: `eyelid surgery Singapore`, `Asian blepharoplasty Singapore`, `double eyelid surgery Singapore`, `ptosis surgery Singapore`
   - Differentiator: Asian eyelid and facial aesthetic training exposure; plastic-surgery background balancing eyelid form and function.

5. Rhinoplasty Singapore
   - Status: drafted for clinical review
   - Target: `rhinoplasty Singapore`, `Asian rhinoplasty Singapore`, `rib rhinoplasty Singapore`, `nose surgery Singapore`
   - Differentiator: Asian rhinoplasty and facial aesthetic training exposure; reconstructive-plastic-surgery background balancing nasal form and function.

6. Body Contouring & Liposuction Singapore
   - Status: drafted for clinical review
   - Target: `body contouring Singapore`, `liposuction Singapore`, `abdominoplasty Singapore`, `tummy tuck Singapore`
   - Differentiator: plastic-surgery approach to contour, scars, tissue quality and staged planning after weight change.

7. Breast Aesthetic Surgery Singapore
   - Status: drafted for clinical review
   - Target: `breast augmentation Singapore`, `breast reduction Singapore`, `breast lift Singapore`

8. Facelift & Facial Rejuvenation Singapore
   - Target: `facelift Singapore`, `facial rejuvenation Singapore`, `deep plane facelift Singapore`

### Tier 3 — referral / authority pages

9. Lower Limb Reconstruction Singapore
   - Target: `lower limb reconstruction Singapore`, `flap surgery Singapore`, `SCIP flap Singapore`
   - Differentiator: SCIP flap book chapter; EFSM 2024 keynote on lower-limb subunit reconstruction.

10. Head and Neck Reconstruction Singapore
   - Target: `head and neck reconstruction Singapore`, `microsurgical reconstruction Singapore`
   - Differentiator: MD Anderson Cancer Center observership exposure.

## Educational article cluster ideas

### Lymphedema cluster

- What is lymphedema surgery?
- Lymphovenous bypass surgery: what patients should know
- LVA vs VLNT: how lymphatic surgery options differ
- Early-stage vs late-stage lymphedema: why assessment matters
- What to expect after lymphatic surgery

### Breast reconstruction cluster

- Breast reconstruction after mastectomy: implant vs flap options
- DIEP flap breast reconstruction: what patients should know
- Timing of breast reconstruction: immediate vs delayed reconstruction

### Aesthetic cluster

- Asian eyelid surgery: consultation, suitability and recovery
- Rhinoplasty in Asian patients: structure, grafts and expectations
- Body contouring after weight loss: what surgery can and cannot address
- Breast augmentation: implant choices, incision planning and recovery

## Recommended first draft sequence

1. Lymphedema Surgery Singapore — published cornerstone procedure page
2. Lymphovenous Bypass / LVA Surgery Singapore — published technical subpage
3. Breast Reconstruction Singapore — published reconstructive authority page
4. Asian Eyelid Surgery Singapore — published aesthetic demand page
5. Rhinoplasty Singapore — drafted aesthetic demand page for clinical review
6. Body Contouring & Liposuction Singapore — drafted aesthetic demand page for clinical review
7. Breast Aesthetic Surgery Singapore — drafted aesthetic demand page for clinical review
8. Facelift & Facial Rejuvenation Singapore — next planned aesthetic demand page
